...
const {
  ...
  NEXT_PUBLIC_SITE_URL,    // e.g. "https://www.turntalks.com"
  USER_EMAIL,
  GMAIL_APP_PASSWORD,
} = process.env;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const linkRes = await fetch(`${BASE_URL}/auth/v1/admin/generate_link`, {
        ...
        type:        "recovery",
        email,
        redirect_to: `${NEXT_PUBLIC_SITE_URL}/auth/reset`,  // ← top‑level!
        }),
    });
    ...
    if (!USER_EMAIL || !GMAIL_APP_PASSWORD) throw new Error("Missing SMTP credentials");
